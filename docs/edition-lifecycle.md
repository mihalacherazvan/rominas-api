...
> [access-control.md](access-control.md)). The remaining hooks — opening the public ballot on
> `voting_open`, locking results for the custodian on `committee_review`, publishing on
> `results_published` — land with the `Voting` / `Results` modules.
>
> **Not** on this event: the per-category nominee **shortlist** is generated by an explicit admin
> action while the edition is `nominations_closed`, not automatically on the transition — see the
> [Shortlist](domain-model.md#shortlist) domain notes and [access-control.md](access-control.md#2e-nominee-shortlist).
