...
> [access-control.md](access-control.md)). The remaining hooks — locking results for the custodian on
> `committee_review`, publishing on `results_published` — land with the `Results` module.
>
...
> `nominations_closed`; and **public voting** is open while the edition is `voting_open` and now is within
